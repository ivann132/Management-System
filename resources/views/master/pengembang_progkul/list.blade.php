                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered zero-configuration">
                                ...
                                <tbody>
                                    ...
                                    $no = 1;
                                    @endphp
                                    @foreach ($data_pengembang_progkul as $row)
                                    <tr>
                                        <td>{{$no++}}</td>
                                        <td>{{$row->deskripsi}}</td>
                                        <td>{{$row->nama_file}}</td>
                                        <td><a href="#modalEdit{{$row->id}}" data-toggle="modal" class="btn btn-xs btn-primary"><i class="fa fa-edit"></i></a>
                                            ...
                                            <a href="{{ asset('penyimpanan/pengembang_progkul/' . $row->nama_file) }}" target="_blank" class="btn btn-info btn-xs"><i class="fa fa-eye"></i> View File</a>
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
...
<div class="modal fade" id="modalCreate" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="/pengembang_progkul/store" method="POST" enctype="multipart/form-data">
                @csrf
...
@foreach ($data_pengembang_progkul as $d)
<div class="modal fade" id="modalEdit{{$d->id}}" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="/pengembang_progkul/update/{{$d->id}}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="modal-body">
                    <div class="form-group">
                        <label>Deskripsi</label>
                        <input type="text" value="{{$row->deskripsi}}" class="form-control" name="deskripsi" placeholder="deskripsi" required>
                    </div>
                    <div class="form-group">
                        <label>Nama File</label>
...
@foreach ($data_pengembang_progkul as $c)
<div class="modal fade" id="modalHapus{{$c->id}}" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="/pengembang_progkul/destroy/{{$c->id}}" method="GET">
                @csrf
                <div class="modal-body">
                    <div class="form-group">
