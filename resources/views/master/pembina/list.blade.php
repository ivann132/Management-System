                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered zero-configuration">
                                ...
                                <tbody>
                                    ...
                                    $no = 1;
                                    @endphp
                                    @foreach ($data_pembina as $row)
                                    <tr>
                                        <td>{{$no++}}</td>
                                        <td>{{$row->nama_penasehat}}</td>
                                        <td>{{$row->tanggal}}</td>
                                        <td>{{$row->no_sk}}</td>
                                        <td>{{$row->nama_file}}</td>
                                        <td><a href="#modalEdit{{$row->id}}" data-toggle="modal" class="btn btn-xs btn-primary"><i class="fa fa-edit"></i></a>
                                            ...
                                            <a href="{{ asset('penyimpanan/pembina/' . $row->nama_file) }}" target="_blank" class="btn btn-info btn-xs"><i class="fa fa-eye"></i> View File</a>
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
            ...
            <form action="/pembina/store" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="nama_penasehat" placeholder="Nama Penasehat" required>
                    </div>
                    <div class="form-group">
                        <label>Tanggal</label>
...
<div class="modal fade" id="modalEdit{{$d->id}}" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="/pembina/update/{{$d->id}}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="modal-body">
                    <div class="form-group">
                        <label>Nama Penasehat</label>
                        <input type="text" value="{{$d->nama_penasehat}}" class="form-control" name="nama_penasehat" placeholder="Nama Penasehat" required>
                    </div>
                    <div class="form-group">
                        <label>Tanggal</label>
...
<div class="modal fade" id="modalHapus{{$c->id}}" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="/pembina/destroy/{{$c->id}}" method="GET">
                @csrf
                <div class="modal-body">
                    <div class="form-group">
